...
##
## <chw@ch-werner.de> modifications for using with sdltk
##
...
proc ::tkcon::Init {args} {
	...
	showOnStartup	1
	slaveprocs	{
	    alias clear dir dump echo idebug lremove tkcon_trim
...
proc ::tkcon::InitUI {title} {
    ...
    grid $sy   -row 1 -column [expr {($OPT(scrollypos) eq "left") ? 0 : 2}] \
	-sticky ns
...
proc ::tkcon::EvalCmd {w cmd} {
	...
	if {$code} {
	    ...
	    if {[string length $res] > 2044} {
		# If we exceed maximum desired output line length, truncate
		# the result and add "..."
		set res "[string range $res 0 2044]..."
	    }
	    if {$code} {
		if {$OPT(hoterrors)} {
		    set tag [UniqueTag $w]
...
proc ::tkcon::EvalSocketClosed {sock} {
    ...
    catch {unset PRIV($sock,socksubst)}
    if {$sock ne $PRIV(app)} {
...
proc ::tkcon::About {} {
	...
	$w.text tag config title -justify center
...
proc ::tkcon::InitMenus {w title} {
    ...
    option add *[string range $w 1 end]*Menu.tearOff 0
...
proc ::tkcon::XauthSecure {} {
    ...
    if {0 && [tk windowingsystem] ne "x11"} {
...
proc ::tkcon::FindBox {w {str {}}} {
	...
	frame $base.btn
	grid $base.l $base.e - - -sticky ew -pady 4 -padx 4
	...
	ttk::button $base.btn.fnd -text "Find" -width 6
	ttk::button $base.btn.clr -text "Clear" -width 6
	ttk::button $base.btn.dis -text "Dismiss" -width 6
...
proc ::tkcon::NewSocket {} {
	...
	ttk::button $t.ok -text "OK" -command {set ::tkcon::PRIV(grab) 1} -width 4 \
	    -takefocus 1
	ttk::checkbutton $t.subst -text "Backslash substitution" \
	    -variable ::tkcon::OPT(socksubst)
	...
	grid $t.subst -columnspan 4 -column 1 -padx 4 -pady 2 -sticky e
	...
	set PRIV($sock,socksubst) $OPT(socksubst)
	Attach $sock socket
    }
}
...
proc ::tkcon::MainInit {} {
	...
	if {[winfo exists $w]} {
	    ...
	    ttk::button $w.check  -text "Recheckpoint" -width 11
...
proc tkcon {cmd args} {
	...
	socksu* {
	    catch {set OPT(socksubst) [expr {[lindex $args 0] != 0}]}
	    return $OPT(socksubst)
	}
	default {
...
## tkcon_trim -
## Trim output string to max. 2k to overcome slowness of text widget
## on very long lines
##
# ARGS:	string to trim
# Outputs:	the trimmed string
##
proc tkcon_trim string {
    set result ""
    foreach line [split $string "\n"] {
	if {[string length $line] > 2044} {
	    lappend result "[string range $line 0 2044]..."
	} else {
	    lappend result $line
	}
    }
...
proc tkcon_puts args {
    ...
    if {$len == 1} {
	tkcon console insert output "[tkcon_trim $arg1]\n" stdout
    } elseif {$len == 2} {
	if {![string compare $arg1 -nonewline]} {
	    tkcon console insert output [tkcon_trim $arg2] stdout
	} elseif {![string compare $arg1 stdout] \
		|| ![string compare $arg1 stderr]} {
	    tkcon console insert output "[tkcon_trim $arg2]\n" $arg1
	} else {
	    set len 0
	}
    } elseif {$len == 3} {
	if {![string compare $arg1 -nonewline] \
	    ...
	    tkcon console insert output [tkcon_trim $arg3] $arg2
	} elseif {(![string compare $arg1 stdout] \
		|| ![string compare $arg1 stderr]) \
		&& ![string compare $arg3 nonewline]} {
	    tkcon console insert output [tkcon_trim $arg2] $arg1
	} else {
	    set len 0
	}
    } else {
