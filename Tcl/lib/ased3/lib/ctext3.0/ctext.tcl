...
proc ctext::instanceCmd {self cmd args} {
	...
	conf* {
		...
		pack $self.l -side left -fill y -before $self.t
	    }
	...
	delete {
		...
		if {![catch {$self._t index "$deleteStartPos lineend"} lineEnd]} {
		    foreach tag [$self._t tag names] {
			if {[string equal $tag "_cComment"] != 1} {
...
proc ctext::matchPair {win str1 str2 escape} {
	...
	if {$found eq "" || [$win compare $found > $pos]} {
	    return
	}

	if {$lastFound ne "" && [$win compare $found == $lastFound]} {
...
proc ctext::matchQuote {win} {
	...
	if {$startQuote eq "" || [$win compare $startQuote > $start]} {
	    # The search found nothing or it wrapped.
	    return
	}

	if {$lastFound ne "" && [$win compare $lastFound == $startQuote]} {
    ...
    if {[$win compare $endQuote eq $startQuote]} {
	# probably just \"
	return
    }
...
proc ctext::comments {win} {
	...
	} elseif {$str eq "/*" && $isQuote == 0 && $isSingleQuote == 0} {
	    if {$isComment} {
		# comment in comment
		break
	    } else {
...
proc ctext::delHighlightClass {win class} {
	...
	if {$class eq [lindex [set ::ctext::highlight${win}($item)] 0]} {
...
proc ctext::delHighlightClassForSpecialChars {win class} {
	...
	if {$class eq [lindex [set ::ctext::highlightSpecialChars${win}($item)] 0]} {
...
proc ctext::delHighlightClassWithOnlyCharStart {win class} {
	...
	if {$class eq [lindex [set ::ctext::highlightCharStart${win}($item)] 0]} {
...
proc ctext::highlight {win start end} {
	...
	catch {$twin tag remove $tag $start $end}
    }
	...
	while {1} {
	    if {[catch {
		$twin search -count length -regexp -- $re $si $end
	    } res]} {
		break
	    }
	    if {$res eq ""} {
...
proc ctext::bind {win args} {
    if {[catch {winfo class $win}] || [winfo class $win ] ne "Ctext"} {
