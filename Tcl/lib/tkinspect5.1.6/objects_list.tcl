...
widget object_list {
    ...
    method update {target} {
        $self clear
        set cmd [list if {[::info command itcl_info] ne {}} {itcl_info objects}]
    ...
    method retrieve_old {target object} {
        ...
        foreach arg $meths {
            if {[string first $class $arg] == 0} {
                ...
                if {$a ne "constructor" && $a ne "destructor"} {
                    set nm "method "
                } else {
                    set nm ""
                }
                if {[lindex $meth 1] ne "<built-in>"} {
