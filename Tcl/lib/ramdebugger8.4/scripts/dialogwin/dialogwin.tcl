...
catch { package require gid_themes }
...
if { [info procs ::gid_themes::GetImage] == "" || ![info exists ::GIDDEFAULT] } {
    #using without GiD gid_themes::GetImage not exists, then return always dialogwinquestionhead
...
# proc DialogWin::messageBox { args } {
    ...
    #     array set opts [list -default "" -icon QuestionWindow.png -message "" -parent . -title "" \
        #         -type ok]
    ...
    #     label $f.l1 -image [gid_themes::GetImage QuestionWindow.png] -grid 0
...
proc MessageWin { text title {image QuestionWindow.png} {parent .} {check_no_more 1}} {
    if { $parent eq "." } { set parent "" }
    set w [dialogwin_snit $parent.%AUTO% -title $title -okname \
            [_ "Ok"] -cancelname "-"]
    set f [$w giveframe]
    
    ttk::label $f.l1 -image [gid_themes::GetImage $image]
...
proc QuestionWin { text {parent .} {id 0}} {
    set check_no_more 0
    if { $id != 0} {
        set check_no_more 1
    }
    if { ![ info exists ::GidPriv(QuestionWin,NoMoreModal$id)]} {
        set ::GidPriv(QuestionWin,NoMoreModal$id) 0
    }
    if { $::GidPriv(QuestionWin,NoMoreModal$id) } {
        WarnWinText "Error: $text"
        set ret 1
    } else {
        set ret [MessageWin $text [_ "Question"] QuestionWindow.png $parent $check_no_more]
        set ::GidPriv(QuestionWin,NoMoreModal$id) $ret
    }
}

proc ErrorWin { text {parent .} {id 0}} {
    set check_no_more 0
    if { $id != 0} {
        set check_no_more 1
    }
    if { ![ info exists ::GidPriv(ErrorWin,NoMoreModal$id)]} {
        set ::GidPriv(ErrorWin,NoMoreModal$id) 0
    }
    if { $::GidPriv(ErrorWin,NoMoreModal$id) } {
        WarnWinText "Error: $text"
        set ret 1
    } else {
        set ret [MessageWin $text [_ "Error"] ErrorWindow.png $parent $check_no_more]
        set ::GidPriv(ErrorWin,NoMoreModal$id) $ret
    }
}

proc WarnWin { text {parent .} {id 0}} {
    if { [info procs GidUtils::IsTkDisabled] != "" && [GidUtils::IsTkDisabled] } {
    ...
    if { $::GidPriv(MessageWin,NoMoreModal$id) } {
        ...
        # set ret [ MessageWin $text [_ "Warning"] QuestionWindow.png $parent $check_no_more]
	# WarnWin should have a Warning icon, or not ?
        set ret [ MessageWin $text [_ "Warning"] WarningWindow.png $parent $check_no_more]
...
proc InfoWin { text {parent .} {id 0}} {
    ...
    if { $::GidPriv(MessageWin,NoMoreModal$id) } {
        WarnWinText "Message: $text"
        set ret 1
    } else {
        set ret [ MessageWin $text [_ "Message"] InfoWindow.png $parent $check_no_more]
        set ::GidPriv(MessageWin,NoMoreModal$id) $ret
    }
    # WarnWinText "$id: after MessageWin: ret=$ret="
}
...
proc CreateWarnWinId { text} {
    ...
    for { set i 0} { $i < [ string length $text]} { incr i} {
        binary scan [ string index $text $i] c byte
        set total_sum [ expr $total_sum + $byte]
        set check_sum [ expr $check_sum ^ $byte]
    }
...
proc WarnWin_hideerror { text errordata { parent .} } {
    ...
    ttk::label $f.l1 -image [gid_themes::GetImage QuestionWindow.png]
    ttk::label $f.msg -justify left -text $text -wraplength 3i
...
proc snit_messageBox { args } {
    ...
    set icons [dict create \
            question "QuestionWindow.png" \
            warning "WarningWindow.png" \
            error "ErrorWindow.png" \
            info "InfoWindow.png"]
    
    if { [dict exists $icons $opts(-icon)] } {
        set opts(-icon) [dict get $icons $opts(-icon)]
    }
    ttk::label $f.l1 -image [gid_themes::GetImage $opts(-icon)]
...
snit::widget dialogwin_snit {
    ...
    constructor args {
        ...
        foreach i $togrid {
            foreach letter $usedletters {
                ...
                bind $i <${::acceleratorKey}-$letter> { continue }
            }
        }
        if { $options(-cancelname) ne "-" } {
    ...
    method createwindownowait_as_toplevel {} {
        ...
        lassign [gid_cross_platform::get_desktop_workarea] scr_x scr_y scr_w scr_h
...
if 0 {
    ...
    ttk::combobox $f.cb1 -textvariable [$w give_uservar newtype $type] -values [list Normal Home] -state readonly
...
# ramsan: i perque toqueu aixo?
if { [catch { package require customLib_utils }] } {
