...
set "macrodata(Insert rectangular text,accelerator)" "<Control-u><Control-i>"
...
set "macrodata(Kill rectangular text,accelerator)" "<Control-u><Control-k>"
...
proc "Comment header" { w } {
    ...
    switch [mc::give_active_file_type] {
        ...
        "XML" - markdown { lassign [list "<!-- " " -->"] prefix suffix }
...
proc open_comment_file { w open_edit } {
    ...
    if { ![file exists $file] && [file exists [file join $dir $file]] } {
    ...
    if { ![file exists $file] } {
        ...
        cd $pwd
        if { !$err && [regexp -line {local-root:\s+(.*)} $ret {} dirF] } {
            if { [file exists [file join [string trim $dirF] $file]] } {
                set file [file join [string trim $dirF] $file]
            }
        }
    }
    if { ![file exists $file] } {
        if { [file pathtype $file] eq "relative" && [file extension $file] in ".md .svg" } {
            set pwd [pwd]
            cd $dir
            ...
            if { [file extension $file] eq ".svg" } {
                puts $fout "<?xml version='1.0' encoding='UTF-8' standalone='no'?>"
                puts $fout "<svg xmlns:svg='http://www.w3.org/2000/svg' width='600' height='300'/>"
            }            
            close $fout
            set file [file join $dir $file]
            cd $pwd
