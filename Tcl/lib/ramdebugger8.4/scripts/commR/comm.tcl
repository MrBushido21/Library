...
#
# RCS: @(#) $Id: comm.tcl,v 1.10 2017-03-30 17:40:36 escolano Exp $

package require Tcl 8

namespace eval ::commR {
    ...
    # in the scope of the procedure '::commR::comm''. This means that
...
proc ::commR::comm_send {} {
    proc send {args} {
	# Use pure lists to speed this up.
	eval [linsert $args 0 ::commR::comm send]
	#eval comm send $args
    }
...
proc ::commR::comm {cmd args} {
    variable comm
    set chan ::commR::comm ; # chan is used in the code of the declared methods.
...
::commR::comm method send {
    ...
    # ramsan
    if { $cmd == "async" && ![info exists comm($chan,peers,$id)] } {
	::commR::commConnectAsyncAndSend $id $args
	return
    }
	...
	vwait ::commR::comm($chan,result,$ser)
	...
	after idle unset ::commR::comm($chan,result,$ser)
...
proc ::commR::commDestroy {} {
    ...
    if {[string compare ::commR::comm $chan]} {
	rename $chan {}
    }
}
...
::commR::commConfVars port p
::commR::commConfVars local b
::commR::commConfVars listen b
::commR::commConfVars socket ro
::commR::commConfVars chan ro
::commR::commConfVars serial ro
...
proc ::commR::commConfigure {{force 0} args} {
    upvar chan chan
    variable comm
    ...
    while {1} {
	set cmd [list socket -server [list ::commR::commIncoming $chan]]
	if {$comm($chan,local)} {
	...
	if {[info exists userport] || ![string match "*already in use" $ret]} {
	    ...
	    if {![string match ::commR::comm $chan]} {
		rename $chan {}
		set ipos [lsearch -exact $comm(chans) $chan]
		# ramsan
		set comm(chans) [lreplace $comm(chans) $ipos $ipos]
	    }
	    error $ret
	}
	set nport [incr comm(lastport)]
...
# RAMSAN
proc ::commR::commConnectAsyncAndSend {id left_args} {
    upvar chan chan
    variable comm
    ...
    fileevent $fid writable [list ::commR::commEndConnectAsyncAndSend $id $fid $chan $left_args]
}

proc ::commR::commEndConnectAsyncAndSend { id fid chan left_args } {
    variable comm

    if { [fconfigure $fid -error] != "" } {
	close $fid
	# it is not considered an error
	#error "error connecting"
	return
    }
    fileevent $fid writable ""
    ...
    puts  $fid [list [list async $ser $left_args]]
    flush $fid
}
...
proc ::commR::commIncoming {chan fid addr remport} {
    variable comm
...
#    fconfigure $fid -trans binary -blocking 0
    fconfigure $fid -blocking 0 -encoding utf-8
    fileevent $fid readable [list ::commR::commCollect $chan $fid]
}
...
proc ::commR::commHook {hook {script +}} {
    upvar chan chan
    variable comm
...
proc ::commR::commCollect {chan fid} {
    variable comm
	...
	if {[info complete $cmd]} {
	    ...
	    after idle \
		    [list ::commR::commExec $chan $fid $comm($chan,fids,$fid) $cmd]
	}
    }
}
...
proc ::commR::commExec {chan fid remoteid buf} {
	...
	vers {
	    set ::commR::comm($chan,vers,$id) $ser
	    return
	}
	default {
...
# proc ::commR::registerwrite { socket port name varname } {

#     if { [fconfigure $socket -error] == "" } {
#         close $socket
#         ::commR::comm new ::commR::comm -port $port -local 1 -listen 1
#         proc ::commR::givename { id } "::commR::comm send \$id RamDebugger::MyNameIs [list $name] $port"
#         commR::comm hook lost {after 2000 set x 1; vwait x}
#         if { $varname != "" } { uplevel \#0 [list set $varname $port] }
#         tk_messageBox -message "port=$port"
#     } else {
#         set fout [open /temp/kk.txt a]
#         puts $fout error=[fconfigure $socket -error]
#         close $fout
#         incr port
#         if { $port >= 12360 } {
#             error "could not register port"
#         }
#         close $socket
#         set socket [socket -async localhost $port]
#         fileevent $socket writable [list ::commR::registerwrite $socket $port $name $varname]
#     }
# }

# proc ::commR::register { name local { varname "" } } {

#     set socket [socket -async localhost 12350]
#     fileevent $socket writable [list ::commR::registerwrite $socket 12350 $name $varname]
# }
...
proc ::commR::register { name local { varname "" } } {
	...
	if { $i == 12350 || [info commands ::commR::comm] eq "" } {
	    ...
	    set err [catch { ::commR::comm config -port $i -local 1 \
		        -listen 1 } errstring]
	}
	if { !$err } {
	    break
...
# RAMSAN
# if {![info exists ::commR::comm(comm,port)]} {
#     if {[string match macintosh $tcl_platform(platform)]} {
#         ::commR::comm new ::commR::comm -port 0 -local 0 -listen 1
#         set ::commR::comm(localhost) \
#                 [lindex [fconfigure $::commR::comm(comm,socket) -sockname] 0]
#         ::commR::comm config -local 1
#     } else {
#         ::commR::comm new ::commR::comm -port 0 -local 1 -listen 1
#     }
# }
