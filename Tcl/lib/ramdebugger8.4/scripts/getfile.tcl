...
proc RamDebugger::GetFile { what types title }  {
    ...
    variable getFile_encrypt_password
    ...
    unset -nocomplain getFile_encrypt_password
    
    ttk::button $f.b2 -image encrypted-16 -style Toolbutton \
        -command [list RamDebugger::GetFile_encrypt -variable \
            ::RamDebugger::getFile_encrypt_password $w]
    tooltip::tooltip $f.b2 [_ "Encrypt file"]
        
    grid $f.l1 $f.e1 $f.b1 $f.b2 -sticky w -padx 2
    grid $f.e1 -sticky ew
...
#     bind $f.e1 <Shift-Home> [list RamDebugger::GetFile_Home desktop]
    ...
    if { $getFile_done } {
        ...
        if { [set! getFile_encrypt_password] ne "" } {
            dict set files_encryption $getfilestring $getFile_encrypt_password
        }
...
proc RamDebugger::GetFile_encrypt { args } {
    ...
    set w [dialogwin_snit $wp._ask -title "Enter password for encrypting file" \
        -class ${::className}w]
    set f [$w giveframe]
    
    ttk::label $f.l1 -text "Password:"
    ttk::entry $f.e1 -textvariable [$w give_uservar password ""] -show *
