...
proc RamDebugger::DisplayVar2 { var X Y x y res } {
    ...
    if { [lindex $res 0] == 0 && [lindex $res 1 0] ne "error" } {
        ...
        incr Y -20
        wm geometry $w +$X+$Y
                $w configure -highlightthicknes 1 -highlightbackground grey -highlightcolor grey
...
proc RamDebugger::DisplayVarWindowEval { what w { res "" } } {
    ...
    if { $what eq "do" } {
        ...
        if { $remoteserverType eq "gdb" } {
            set remoteserver [lreplace $remoteserver 3 3 [list getdata [list RamDebugger::DisplayVarWindowEval res $w]]]            
            set comm ""
...
proc RamDebugger::_chooseColor { w button what } {
    set col [tk_chooseColor -initialcolor [$w give_uservar_value colors,$what] -parent $button -title "Choose color"]
    if { $col ne "" } {
        $w set_uservar_value colors,$what $col
                $button configure -background $col -text $col -foreground [_giveopposite_color $col]
    }
}
...
proc RamDebugger::increase_decrease_text_font { increase_decrease args } {
    ...
    foreach fam [list FixedFont NormalFont] {
        ...
        set size [expr {[font configure $fam -size]+$delta}]
        if { $size < 4 } {
            set size 4
        }
...
proc RamDebugger::CheckAutoSavePreference { value} {
    if { $value == 1} {
        set err [ catch {
            RamDebugger::VCS::Init
        } err_txt]
        if { $err} {
            WarnWin [_ "Auto save will fail. Reason: %s" $err_txt]
        }
    }
}
...
proc RamDebugger::PreferencesWindow {} {
    ...
    ttk::checkbutton $lb.c1 -text [_ "Perform auto save revisions"] -variable \
        [$w give_uservar AutoSaveRevisions] -command "RamDebugger::CheckAutoSavePreference $[$w give_uservar AutoSaveRevisions]"
            ...
            1 - 2 {
                ...
                foreach "curr curr_as dir args tcl_or_tk" $options(currentfileargs5) {
                    if { $curr == [$w give_uservar_value curr] } {
                        set options(currentfileargs5) [lreplace $options(currentfileargs5) $ipos [expr $ipos+4]]
                        break
                    }
                    incr ipos 5
                }
        ...
        while { [gets $fin line] > -1 } {
            ...
            if { $ret } {
                ...
                set path [file dirname $path]
                                ...
                                lassign  [list "" "" ""] file path linenum
                set txt $line
            }
...
proc RamDebugger::SearchInFilesGo { w tree ids } {
    ...
    if { $ids eq "" } { return }
    lassign [$tree item text [lindex $ids 0]] file line txt path
...
proc RamDebugger::SearchWindow { args }  {
    ...
    variable iswince
    #WarnWinText "RamDebugger::SearchWindow $args"   
    set optional {
        { -replace boolean 0 }
    ...
    if { $auto_close != 0 } {
        #auto_close==1 or auto_close=="force"
        if { $auto_close == "force" || $options(SearchToolbar_autoclose) } { 
            #it is forced or the option allow close
            #close
            ...
            # option doesn't allow close and it is not forced
            # do not close                                         
        }
        return
    } else {
        ...
        if { [info exists SearchToolbar] } {
            ...
            if { $replace != [lindex $SearchToolbar 1] } {
                ...
                if { [lindex $SearchToolbar 0] } {
                    #find is visible            
#                     if { 0 } {
#                         #disable this code that swap SearchToolbar_autoclose in an unwanted way!!
...
#                     }
                    ...
                    # find is not  visible  
#                     if { 0 }  {
#                         #this code was disable from years ago...
#                         tkTabToWindow $f.e1                    
...
#     }
    ...
    if { $istoplevel } {
          bind [winfo toplevel $f] <Destroy> [list RamDebugger::SearchWindowOnDestroy %W [winfo toplevel $f] $active_text 0]
        DialogWinTop::CreateWindow $f
    } else {
          bind $f <Destroy> [list RamDebugger::SearchWindowOnDestroy %W $f $active_text 1]
    }
}

proc RamDebugger::SearchWindowOnDestroy { W w active_text unset_searchtoolbar } {        
        if { $W != $w } {
                #to ensure do it only once
                return
        }        
        if { [winfo exists $active_text] } {
          $active_text tag remove search 1.0 end
        }
        if { $unset_searchtoolbar } {
      unset ::RamDebugger::SearchToolbar
    }
}
    ...
    switch $what {
        ...
        replace {
            if { [llength [$text tag ranges search]] == 0 } {
                Search $w beginreplace
                        ...
                        lassign [$text tag ranges search] idx1 idx2
        ...
        replaceall {
            ...
            while 1 {
                ...
                if { $searchmode == "-regexp" } {
                    if { $searchcase } {
                                        regsub $searchstring [$text get $idx1 $idx2] $replacestring txt
                    } else {
                                        regsub -nocase $searchstring [$text get $idx1 $idx2] $replacestring txt
                    }
                } elseif { !$searchcase && \
                    [string tolower $searchstring] eq $searchstring && \
                    [string tolower $replacestring] eq $replacestring } {
        ...
        contextual {
                        lassign $args replace x y
            set menu $w._menu
            catch { destroy $menu }
            menu $menu -tearoff 0
            if { !$replace } {
                                $menu add command -label Replace -command [list RamDebugger::SearchWindow -replace 1]
            } else {
                                $menu add command -label Search -command [list RamDebugger::SearchWindow]
            }
            $menu add separator
                        $menu add radio -label Backward -variable ::RamDebugger::SearchType -value -backwards
                        $menu add radio -label Forward -variable ::RamDebugger::SearchType -value -forwards
            $menu add separator
                        $menu add check -label "From beginning" -variable ::RamDebugger::searchFromBegin
                        $menu add check -label "Consider case" -variable ::RamDebugger::searchcase
                        $menu add check -label "Regexp mode" -variable ::RamDebugger::searchmode -onvalue -regexp -offvalue -exact
            $menu add separator
            ...
            $menu add separator
            if { !$replace } {
                                $menu add command -label Close -command [list RamDebugger::SearchWindow]
            } else {
                                $menu add command -label Close -command [list RamDebugger::SearchWindow -replace 1]
            }
            tk_popup $menu $x $y
        }
    }
...
proc RamDebugger::AddActiveMacrosToMenu { mainframe menu } {
    ...
    interp alias "" Macros::mc::process_markdown "" RamDebugger::process_markdown
