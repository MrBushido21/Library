...
proc RamDebugger::VCS::GetVCSCommand {} {
    variable cvs_or_fossil
    set vcs_command ""
    if { $cvs_or_fossil eq "cvs" } {
        set vcs_command [ auto_execok cvs]
    } else {
        set vcs_command [ auto_execok fossil]
    }
    return $vcs_command
}
...
proc RamDebugger::VCS::indicator_menu { cvs_indicator_frame x y } {
	...
	[list RamDebugger::VCS::update_recursive_cmd "" open_program fossil_ui "" \
	    current -file $currentfileL]
...
#     mylog::init -view_binding <Control-L> debug
    ...
    bind $w <$::control-a> [list "update_recursive_cmd" $w open_program add_fossil $f.toctree ""]
...
proc RamDebugger::VCS::update_recursive_cmd { w what args } {
	...
	contextual {
	    ...
	    if { $fossil_active } {
		$menu add command -label [_ "Add files"] -accelerator $::control_txt-a -command \
		    [list "update_recursive_cmd" $w add_fossil $tree $sel_ids]
		if { !$can_be_added } {
		    $menu entryconfigure end -state disabled
		}
		$menu add command -label [_ "Add remove..."] -command \
		    [list "update_recursive_cmd" $w add_remove $tree $sel_ids $dirs]
	    }
		...
		fossil_ui {
		    ...
		    if { $file ne "" } {
		        ...
		        if { !$err } {
		            ...
		            if { $ret } {
		                if { [RamDebugger::GiveFileType $file] eq "wiki" } {
		                    set url_suffix "/doc/ckout/$sfile"
		                } else {
	...
	add_remove {
	    lassign $args tree sel_ids dirs
	    set dir [lindex $dirs 0]
	    waitstate $w on sync
	    get_cwd
	    cd $dir
	    set fossil [auto_execok fossil]
	    set err [catch { exec $fossil addremove --dry-run } ret]
	    if { $err } {
		waitstate $w off
		release_cwd
		snit_messageBox -message $ret -parent $w
	    ...
	    set wD $w.diffs
	    destroy $wD
	    dialogwin_snit $wD -title [_ "Add & remove"] -class $::className -entrytext \
		[_ "Potencial results for addremove on '%s'" $dir] \
		-grab 0 -transient 1 -callback [namespace code [list "update_recursive_cmd" \
		        $w add_remove_accept $dir]]
	    set f [$wD giveframe]
	    
	    set columns [list \
		    [list  20 [_ "what"] left text 0] \
		    [list 80 [_ "file"] left text 0] \
		    ]
	    fulltktree $f.lf -width 600 -height 600 \
		-columns $columns -expand 0 \
		-selectmode extended -showheader 1 -showlines 0  \
		-indent 0 -sensitive_cols all \
		-contextualhandler_menu [list "update_recursive_cmd" $w \
		    add_remove_contextual $dir] \
		-selecthandler2 "[list $wD invokeok];#"
	    $wD set_uservar_value tree $f.lf
	    
	    foreach i [split $ret \n] {
		regexp {^(\w+)\s+(.*)} $i {} cmd file
		$f.lf insert end [list $cmd $file]
	    }
	    focus $f.lf
	...
	add_remove_contextual {
	    lassign $args dir tree menu id sel_ids
	    set files ""
	    foreach item $sel_ids {
		set txt [$tree item text $item 1]
		set file [file join $dir $txt]
		if { [file exists $file] } {
		    lappend files $file
		}
	    }
	    $menu add command -label [_ "Delete"] -command \
		[list "update_recursive_cmd" $w add_remove_delete $tree $sel_ids $files]
	}
	add_remove_delete {
	    lassign $args tree sel_ids files
	    if { 0 } {
		set text [_ "Do you want to send '$files' to recycle bin?"]
	...
	add_remove_accept {
		...
		cd $dir
		set fossil [auto_execok fossil]
		set err [catch { exec $fossil addremove } ret]
		if { $err } {
		    waitstate $w off
		    release_cwd
		    snit_messageBox -message $ret -parent $w
