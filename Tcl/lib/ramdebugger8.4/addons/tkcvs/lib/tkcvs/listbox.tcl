...
#
# RCS: @(#) $Id: listbox.tcl,v 1.3 2012-04-18 17:28:36 escolano Exp $
#
...
#RAMSAN
proc tkListboxKeyPress {w key} {
    global tkPriv

    if { $key == "" } { return }
    append tkPriv(ListboxAccel,$w) [string tolower $key]
    set s [$w size]
    set curr [$w index anchor]
    incr curr
    set found 0
    for { set i $curr } { $i < $s } { incr i } {
	set tryfile [string tolower [$w get $i]]
	if { [string match $tkPriv(ListboxAccel,$w)* $tryfile] } {
	    set found 1
	    break
	}
    }
	...
	after cancel $tkPriv(ListboxAccel,$w,afterId)
    }
    set tkPriv(ListboxAccel,$w,afterId) [after 500 tkListboxKeyPress_Reset $w]
}
proc tkListboxKeyPress_Reset {w} {
    global tkPriv

    catch {unset tkPriv(ListboxAccel,$w)}
}
