...
# mylog::init -view_binding <Control-L> debug
    ...
    package require -exact customLib_utils::c 1.15
}
...
proc RamDebugger::Init { _readwriteprefs _prefs_group { registerasremote 1 } { _big_icons 0 } 
    ...
    # called from GiD, may be NormalFont & co. already exists, mainting gui"
    if { [info exists ::is_package] && $::is_package } {
        foreach fnt [ list NormalFont FixedFont HelpFont] {
            catch {
                set options_def($fnt) [ font configure $fnt]
            }
        }
    }
    ...
    set "options_def(extensions,wiki)" ".wiki .json"
    set "options_def(extensions,markdown)" ".md"
    set "options_def(extensions,todo.txt)" "todo.txt"
    ...
    # enable only if there is a program to use for the autosave functionality
    set autosave_cmd [ RamDebugger::VCS::GetVCSCommand]
    if { $autosave_cmd != ""} {
        set options_def(AutoSaveRevisions) 1
    } else {
        set options_def(AutoSaveRevisions) 0
    }
...
proc RamDebugger::rhelp { args } {
    ...
    RamDebugger::ParseArgs $args $usagestring opts
...
proc RamDebugger::rdebug { args } {
    ...
    if { $opts(-master) } {
        #do this only if force, to avoid lock opening ramdebugger inside GiD
        # nothing
    } elseif { $opts(-forceupdate2) } {
        RamDebugger::FindActivePrograms 2
    } elseif { $opts(-forceupdate) } {
        RamDebugger::FindActivePrograms 1
    } else { 
        RamDebugger::FindActivePrograms 0 
    }
        ...
        if { [info exists options(currentfileargs5)] } {
            foreach "curr curr_as dir_in arg_in tcl_or_tk" $options(currentfileargs5) {
                if { $curr == $currentfile } {
                    ...
                    RamDebugger::TextOutInsertBlue $txt
                    RamDebugger::SetMessage [_ "Executing '%s'" $filetodebug]
                    local eval [list set argc [llength $arg_in]]
                    local eval [list set argv $arg_in]
                    RamDebugger::TextOutInsertBlue [_ "Using arguments: '%s'\n" $arg_in]
                    if { $tcl_or_tk != "auto" } {
                        set LocalDebuggingType $tcl_or_tk
                        RamDebugger::TextOutInsertBlue [_ "Considering file as type: %s" $tcl_or_tk]
                    }
                    RamDebugger::TextOutInsertBlue [_ "Defined in: File->Debug on->Current file arguments\n"]
                    break
                }
            }
        }
    ...
    set remotecomm {
        ...
        proc RDC::GetInternalRepresentation { x } {
            set type ""
...
proc RamDebugger::rstack { args } {
    ...
    set comm {
        ...
        for { set ::RDC::i $::RDC::current } { $::RDC::i > 0 } { incr ::RDC::i -1 } {
            set ::RDC::info_level_i [info level $::RDC::i]
            set ::RDC::level [lindex $::RDC::info_level_i 0]
            foreach ::RDC::j [lrange $::RDC::info_level_i 1 end] {                           
                set ::RDC::internal_representation [RDC::GetInternalRepresentation $::RDC::j]
                if { $::RDC::internal_representation == "int" || $::RDC::internal_representation == "double" } {
                    #do nothing
                } elseif { $::RDC::internal_representation == "cmdName" || $::RDC::internal_representation == "parsedVarName" } {
                    #do nothing
                } elseif { $::RDC::internal_representation == "path" } {  
                    #do nothing
                } elseif { $::RDC::internal_representation == "list" } {
                    if { [llength $::RDC::j] > 10 } {
                        set ::RDC::j [lrange $::RDC::j 0 9]...
                    }
                } elseif { $::RDC::internal_representation == "dict" } {
                    #do nothing
                } elseif { $::RDC::internal_representation == "string" || $::RDC::internal_representation == "pure string" } {
                regsub -all {\n} $::RDC::j { } ::RDC::j
                if { [string length $::RDC::j] > 100 } {
                    set ::RDC::j [string range $::RDC::j 0 96]...
                }
                } elseif { [string match *array $::RDC::internal_representation] } {
                    if { [objarray length $::RDC::j] > 10 } {
                        set ::RDC::j [objarray range $::RDC::j 0 9]...
                    }
                } else {
...
proc RamDebugger::rlist { args } {
    ...
    if { ![info exists files($currentfile)] || $force} {
        ...
        set encoding ""
        if { [GiveFileType $currentfile] in "wiki markdown todo.txt" } {
            set encoding utf-8
        }
        if { [file extension $currentfile] in ".yaml" } {
            set encoding utf-8
        }
        if { $opts(-encoding) != 0 && $opts(-encoding) != "" } {
            set encoding $opts(-encoding)
        }
        ...
        if { [regexp {^-----BEGIN MYSECRET-----} $header] } {
            variable text
            if { [dict exists [set! files_encryption] $currentfile] } {
                set password [dict get $files_encryption $currentfile]
            } else {
                set password [GetFile_encrypt $text]
            }
            set files($currentfile) [encoding convertfrom utf-8 [exec MySecret.exe -d -@ \
                        -p $password -n << $files($currentfile)]]
            dict set files_encryption $currentfile $password
        }
    ...
    if { ($filetype == "TCL"  && ![info exists instrumentedfilesP($currentfile)]) || \
        ...
        RamDebugger::SetMessage [_ "Instrumenting file '%s'" $currentfile]...
        ...
        if { $filetype eq "wiki" } {
            set err [catch { Instrumenter::DoWorkForWiki $files($currentfile) \
                        instrumentedfilesInfo($currentfile) } errstring]
            if { $err } {
                set einfo $::errorInfo
                RamDebugger::ProgressVar 100
        ...
        if { $filetype eq "markdown" } {
            set err [catch { Instrumenter::DoWorkForMarkdown $files($currentfile) \
                        instrumentedfilesInfo($currentfile) } errstring]
            if { $err } {
                set einfo $::errorInfo
                RamDebugger::ProgressVar 100
        ...
        foreach i [list P R] {
            ...
            if { $instfile != "" && [info exists instrumentedfiles${i}($currentfile)] && \
                ...
                if { $encoding eq "" } {
                    set header [string range [set instrumentedfiles${i}($currentfile)] 0 2000]
                    if {[string range $header 0 2] eq [binary format ccc 0xEF 0xBB 0xBF] } {
                        set encoding utf-8
                    }
                    set rex {-\*-.*coding:\s*utf-8\s*;.*-\*-|encoding=['\"]utf-8['\"]}
                ...
                if { $encoding ne "" } {
                    fconfigure $fout -encoding $encoding
                }
...
if { $debuggerstate == "time" && [info exists files($currentfile)] && \
    ...
    RamDebugger::SetMessage [_ "Instrumenting file '%s' for time measure" $currentfile]...
...
proc RamDebugger::GiveFileType { args } {
    variable options
    variable options_def
    ...
    if { !$no_default && $options(filetype) ne "auto" } {
...
proc RamDebugger::MyNameIs { name id } {
    variable services
    RamDebugger::SetMessage [_ "Remote program: %s" $name]
...
proc RamDebugger::FindActivePrograms { force } {
    ...
    if { $usecommR } {
        ...
        if { [info exists services] && !$force } { 
            return 
        }
        catch { unset services }
        RamDebugger::SetMessage [_ "Searching external programs"]...        
        ...
        for { set i 12350 } { $i < 12360 } { incr i } {
            ...
            set comm [list commR::comm send -async $i [list catch [list commR::givename $debuggerserverNum]]]
            if { [info exists ::is_package] && $::is_package } {
                #do nothing, otherwise wait forever
            } else {
...
proc RamDebugger::ReceiveFromProgramSource { args } {
    ...
    if { $retval == 1 } {
        RamDebugger::TextOutRaise
        RamDebugger::TextOutInsertBlue [_ "Sending Instrumented file '%s'" $file]\n
        ...
        RamDebugger::TextOutRaise
        RamDebugger::TextOutInsertBlue [_ "Sourcing file '%s'" $file]\n
    }
...
#     }
    ...
    process_markdown_check_delete_tmpfiles -force
...
proc RamDebugger::ReadPreferences {} {
    ...
    if { [info exists ::is_package] && $::is_package } {
        foreach fnt [ list NormalFont FixedFont HelpFont] {
                ...
                set options($fnt) [ font configure $fnt]
            }
        }
    }
}
...
proc RamDebugger::OpenFileF { args } {
    ...
    if { [GiveFileType $currentfile] eq "todo.txt" } {
        if { [GiveFileType -no_default $file] eq "todo.txt" } {
            process_todo_txt_end -noclosebar
        } else {
    ...
    if { $filetype eq "markdown" } {
        markdown_latex_formulae
    }
    if { $filetype eq "todo.txt" } {
        process_todo_txt
    }
    if { [info exists currentfile_secondary] } {
    ...
    RamDebugger::WaitState 0
    if { $set_focus && ([focus -lastfor $text] eq $text || \
...
proc RamDebugger::CloseFile {} {
    ...
    if { [dict exists [set! files_encryption] $currentfile] } {
        dict unset files_encryption $currentfile
    }
}
...
proc RamDebugger::_savefile_only { args } {
    ...
    if { $file eq "*Macros*" } {
        ...
        set encoding ""
        if { [GiveFileType $file] in "wiki markdown todo.txt" } {
            set encoding utf-8
        }
        if { $encoding eq "" } {
            set header [string range $data 0 2000]
            if {[string range $header 0 2] eq [binary format ccc 0xEF 0xBB 0xBF] } {
                set encoding utf-8
            }
            set rex {-\*-.*coding:\s*utf-8\s*;.*-\*-|encoding=['\"]utf-8['\"]}
        ...
        if { [dict exists [set! files_encryption] $file] } {
            set password [dict get $files_encryption $file]
            set data [exec MySecret.exe -e -@ -p $password -n << [encoding convertto utf-8 $data]]
        }
...
proc RamDebugger::SaveFileF { args } {
    ...
    RamDebugger::WaitState 1
    RamDebugger::SetMessage [_ "Saving file '%s'" $file]...
    ...
    RamDebugger::VCS::indicator_update
    RamDebugger::WaitState 0
    RamDebugger::SetMessage [_ "Saved file '%s'" $file]
}
...
#         $menu add radio -label [_ "Send all"] -variable RamDebugger::remoteserver -value "master all"
    ...
    RamDebugger::WaitState 1
    RamDebugger::SetMessage [_ "Searching for active programs"]...
...
proc RamDebugger::CheckEvalEntries { what { name "" } { res "" } } {
    ...
    if { $name == "" } {
        ...
        if { $what == "do" } {
            ...
            if { $remoteserverType == "gdb" } {
                set remoteserver [lreplace $remoteserver 3 3 [list getdata "RamDebugger::CheckEvalEntries res $name"]]
                set comm "output $var\n"
            ...
            set comm {
                ...
                foreach ::RDC::i [info locals] {
                    if { [array exists $::RDC::i] } {
                        ...
                        set ::RDC::internal_representation [RDC::GetInternalRepresentation $::RDC::val]
                        if { [string match *array $::RDC::internal_representation] } {
                            if { [objarray length $::RDC::val] > 10 } {
                                set ::RDC::val [objarray range $::RDC::val 0 9]...
                            }
                        } elseif { $::RDC::internal_representation == "list" } {
                            if { [llength $::RDC::val] > 10 } {
                                set ::RDC::val [lrange $::RDC::val 0 9]...
                            }
                        } elseif { $::RDC::internal_representation == "string" || $::RDC::internal_representation == "pure string" } {
                        if { [string length $::RDC::val] > 100 } {
                            set ::RDC::val [string range $::RDC::val 0 96]...
...
proc RamDebugger::TextOutRaiseDoForce { pane } {
    variable options
...
proc RamDebugger::paste_image_selection_or_file_get_dir { w } {
...
proc RamDebugger::paste_image_selection_or_file { args } {
    variable text
    variable currentfile
    variable paste_image_selection_or_file_dirList
    variable paste_image_selection_or_file_file
    ...
    if { $fileFrom ne "" && $filetype ne "markdown" } {
        return 0
    }
    ...
    if { [info exists paste_image_selection_or_file_dirList] } {
        set dirList $paste_image_selection_or_file_dirList
    } else {
        set dirList ""
    }
    if { [file dirname $currentfile] ni $dirList } {
        lappend dirList [file dirname $currentfile]
        lappend dirList [file join [file dirname $currentfile] images]
    }
...
#     [[Image:image_name.png|...]]
    ...
    set has_regexp 0
    if { [regexp $rex $txt {} f1 f2 f3 f4 f5] } {
        if { $f1 eq "" } {
            set f1 $f2
            if { $f1 eq "" } {
                set f1 $f3
    ...
    if { $fileFrom ne "" } {
        lappend filesList [file tail $fileFrom]
    } elseif { [info exists paste_image_selection_or_file_file] } {
        set d 1
        set fileL [file tail [file root $paste_image_selection_or_file_file]]
        regexp {^(.*[^\d])(\d+)$} $fileL {} fileL d
        incr d
        lappend filesList "$fileL$d.png"
    } else {
    ...
    set f [$w giveframe]
    if { $fileFrom ne "" } {
        set txt [_ "Save image to file"]
    } else {
        set txt [_ "Save clipboard image to file"]
    }
    label $f.l1 -text $txt
    ...
    ttk::button $f.b1 -image [Bitmap::get folder] -command \
        [list RamDebugger::paste_image_selection_or_file_get_dir $w] \
        -style Toolbutton
    ttk::combobox $f.e2 -textvariable [$w give_uservar file $file] \
        -values $filesList
    
    if { $filetype eq "markdown" } {
        ttk::checkbutton $f.cb1 -text [_ "Embbed image"] -variable [$w give_uservar embbed 0]
        $w enable_disable_on_key embbed [dict create 0 "$f.e1 $f.b1 $f.e2"]
    } else {
        $w set_uservar_value embbed 0
    }
    grid $f.l1 - -sticky w -padx 2 -pady 5
    grid $f.e1 $f.b1 -sticky ew -padx 2  -pady 2
    grid $f.e2 -sticky ew -padx 2 -pady 2
    if { $filetype eq "markdown" } {   
        grid $f.cb1 - -sticky w -padx 2 -pady 5
    }
    ...
    while 1 {
        ...
        set embbed [$w give_uservar_value embbed]
        if { $embbed == 0 } {
            set dir [$w give_uservar_value dir]
            set file0 [$w give_uservar_value file]
            set fileTo [file join $dir $file0]
            if { ![file isdirectory [file dirname $fileTo]] } {
            ...
            if { [file exists $fileTo] } {
                set t [_ "File '%s' exists. Overwrite?" $fileTo]
            ...
            if { $fileFrom ne "" } {
                set err [catch { file copy -force $fileFrom $fileTo } errstring]
            } else {
                set err [catch { $img write $fileTo -format png } errstring]
            }
            if { $err } {
                destroy $w
            ...
            if { $fileFrom ne "" } {
                set fin [open $fileFrom rb]
                set data [read $fin]                
                set data [binary encode base64 $data]
                close $fin
                set type [string tolower [string trim [file extension $fileFrom] .]]
            } else {
                set data [$img data -format png]
                set type png
            }
            set fileTo "<data:image/$type;base64,$data>"
        }
        if { $fileFrom eq "" } {
            image delete $img
        }
    ...
    if { $embbed == 0 } {
        set paste_image_selection_or_file_dirList [linsert0 $dirList [file dirname $fileTo]]
        set paste_image_selection_or_file_file $fileTo
    }
    if { !$has_regexp && $filetype eq "latex" } {
        set f0 [file root [file tail $fileTo]]
    ...
    } elseif { !$has_regexp && $filetype eq "markdown" } {
        set fileTo [fileutil::stripPath [file dirname $currentfile] $fileTo]
        $text insert insert "!\[\]($fileTo)\n"
    ...
    RamDebugger::SetMessage [_ "Created image file '%s'" $fileTo]
    return 1
}
        ...
        paste {
            ...
            if { [package vcompare $::tcl_version 8.4] >= 0 } {
                ...
                if {![catch {::tk::GetSelection $text CLIPBOARD} sel]} {
                    ...
                    if { !$err && "image/x-inkscape-svg" in [cu::img::clipboard types] } {
                        auto_load ::svgml::paste
                        if { [info command ::svgml::paste] ne "" } {
                            set svg [cu::img::clipboard get -type image/x-inkscape-svg]
                            svgml::paste $svg
                            return
                        }
                    }
                    if { !$err && $is_image } {
                        paste_image_selection_or_file
                    }
                }
            } else {
                tk_textPaste $text
...
#     }
    ...
    switch $filetype {
        ...
        markdown {
            if { [IsCurrentPositionSVGML] } {
                set svgml 1
            } else {
                set svgml 0
            }
            set txt [$text get "$l1_old.0 -1l linestart" "$l1_old.0 -1l lineend"]
            if { [regexp {\\\s*$} $txt] } {
                set block "$txt\n$block"
                incr l1_old -1
                incr l1_new -1
            }
            set err [catch { Instrumenter::DoWorkForMarkdown -svgml $svgml $block blockinfo } errstring]
            set oldlevel 0
        }
        "GiD BAS file" {
...
proc RamDebugger::_search_braces_and_select { openL closeL x y } {
    ...
    if { $error } { 
        RamDebugger::SetMessage [_ "Error: braces not OK"] 
    }
...
proc RamDebugger::CommentSelection { what } {
    ...
    if { $filetype == "C/C++" } {
        ...
        set commentchar "*#"
    } elseif { $filetype == "XML" || [file extension $currentfile] eq ".md" } {
        set commentchar "<!--"
        set end_commentchar "-->"
...
proc RamDebugger::TrimLines {} {
    ...
    lassign [$text tag nextrange sel 1.0 end] sel1 sel2
    if { $sel1 ne "" } {
        scan $sel1 "%d" line1
        scan $sel2 "%d" line2
    } else {
    ...
    while { $line1 <= $line2 } {
        ...
        scan [$text index end-1c] "%d.%d" line_end pos
        for { set i $line1 } { $i > 1 } { incr i -1 } {
            set i_prev [expr {$i-1}]
            if { [regexp {^\s*$} [$text get "$i_prev.0" "$i_prev.0 lineend"]] } {
                set line1 $i
                break
            } elseif { [regexp {^\s*(-|:|\d+\.)\s+} [$text get "$i.0" "$i.0 lineend"]] } {
                set line1 $i
                break
            }
        }
        for { set i $line1 } { $i <= $line_end } { incr i } {
            if { [regexp {^\s*$} [$text get "$i.0" "$i.0 lineend"]] } {
                if { $i == $line1 } {
                    set line1 [expr {$i+1}]
                } else {
                    set line_end [expr {$i-1}]
                    break
                }
            } elseif { [regexp {^\s*(-|:|\d+\.)\s+} [$text get "$i.0" "$i.0 lineend"]] } {
                if { $i == $line1 } {
        ...
        if { $contract_expand eq "" } {
            if { $line1 == $line_end } {
                set contract_expand expand
            } else {
                set contract_expand contract
            }
        }
        
        set txt [$text get "$line1.0" "$line_end.0 lineend"]
        regsub -all {\s+} $txt { } txt
        if { $contract_expand eq "expand" } {
            if { [regexp {^\s*(-|:|\d+\.)\s+} $txt p] } {
                set prefix [string repeat " " [string length $p]]
            } else {
                set prefix ""
            }
            lassign "0 0 0 {}" startL c numlines txtN
            while { [regexp -indices -start $c {\s} $txt idxs] } {
                lassign $idxs c1 c2
                if { $c1-$startL > 75 } {
                    append txtN [string range $txt $startL $c1] "\n$prefix"
                    set startL [expr {$c2+1}]
                    incr numlines
                }
                set c [expr {$c2+1}]
            }
            append txtN [string range $txt $startL end]
            incr numlines
            set isin 0
            set after 0
            if { $sel2 ne "" && [$text compare $sel2 >= "$line1.0"] &&
                [$text compare $sel2 <= "$line_end.0 lineend"] } {
                set isin 1
            }
            if { $sel2 ne "" && [$text compare $sel2 > "$line_end.0 lineend"] } {
                set after 1
            }
            $text replace "$line1.0" "$line_end.0 lineend" $txtN
            set line1 [expr {$line_end+$numlines}]
            if { $isin } {
                set sel2 [$text index "$sel2+[expr {$numlines-1}] l lineend"]
            } elseif { $after } {
                set sel2 [$text index "$sel2+[expr {$numlines-1}] l"]
            }
        } else {
            set isin 0
            set after 0
            ...
            set numlines [expr {$line_end-$line1}]
            $text replace "$line1.0" "$line_end.0 lineend" $txt
            if { $isin } {
                set sel2 [$text index "$line1.0 lineend"]
            } elseif { $after } {
                set sel2 [$text index "$sel2-$numlines l"]
            }
            set line1 [expr {$line_end+1}]
        }
    }
    if { $sel1 ne "" } {
        $text tag remove sel 1.0 end
        $text tag add sel $sel1 $sel2
    }
    #SetIsModified
}
...
proc RamDebugger::IsCurrentPositionSVGML {} {
    variable text
    
    regexp {^\d+} [$text index insert] line
    for { set i [expr {$line-1}] } { $i >= 1 && $i >= $line-100 } { incr i -1 } {
        set txt [$text get "$i.0" "$i.0 lineend"]
        if { [regexp {^\s*~{3,}.*svgml} $txt] } {
            return 1
        } elseif { [regexp {^\s*~{3,}} $txt] } {
            return 0
        }
    }
    return 0
...
proc RamDebugger::IndentLineMarkdown { line pos } {
    variable text
    variable instrumentedfilesInfo
    variable currentfile
    ...
    lassign [lindex $instrumentedfilesInfo($currentfile) [expr $line-1]] level type
    ...
    if { $type ni "n c" } { return }
    if { $type eq "n" && ![regexp {^(\S+)\s+(?:([+\w]+)\s+)?(.*)$} $txt {} a b c] } {
        return
    }
    if { $type eq "c" && ![regexp {^\s*(\S.*)} $txt {*} a] } {
        return
    }
    if { $type eq "c" } {
        set tabs 0
        set txt [string repeat " " 22]
        lappend tabs [string length $txt]
        append txt "$a"
    } else {
        set txt "$a"
        if { [string length $a] > 14 } {
            append txt " "
        } else {
            append txt [string repeat " " [expr {15-[string length $a]}]]
        }
        set tabs 0
        if { $b ne "" } {
            lappend tabs [string length $txt]
            ...
            if { [string length $b] > 6 } {
                ...
                append txt [string repeat " " [expr {7-[string length $b]}]]
            }
        }
        lappend tabs [string length $txt]
        append txt "$c"
    ...
    if { $pos >= 0 && $pos < [string length $txt] } {
        for { set i 1 } { $i < [llength $tabs] } { incr i } {
            if { $pos > [lindex $tabs $i-1] && $pos < [lindex $tabs $i] } {
                set pos [lindex $tabs $i]
                break
            }
        }
        $text mark set insert $line.$pos
    }
}
...
proc RamDebugger::IndentLine { line { pos -1 } } {
    ...
    if { $filetype == "C/C++" } {
        ...
        set indent_val 0
    } elseif { $filetype eq "markdown" } {
        IndentLineMarkdown $line $pos
        return
    } else { return }
...
proc RamDebugger::UpdateLineNumDo {} {
    ...
    if { $currentfile ne "" && [string index $currentfile 0] != "*" } {
        ...
        if { $CheckExternalFileModification != 0 && $exists && $mtime > $filesmtime($currentfile) } {
            set filesmtime($currentfile) $mtime
            
            if { $CheckExternalFileModification eq "auto" && !$currentfileIsModified } {
                OpenFileF -force 1 -set_focus 0 $currentfile
                return
            }
            ...
            set w $text._ask
            dialogwin_snit $w -title [_ "Reload file"] -class $::className
            set f [$w giveframe]
            ttk::label $f.l1 -text $quest -wraplength 400 -justify left
            ttk::checkbutton $f.cb1 -text [_ "Reload automatically"] \
                -variable [$w give_uservar auto 0]
            ttk::checkbutton $f.cb2 -text [_ "Do not reload and disable warning"] \
                -variable [$w give_uservar disable 0]

            grid $f.l1 -sticky nw -padx 5 -pady 5
            grid $f.cb1 -sticky nw -padx 5 -pady "5 1"
            grid $f.cb2 -sticky nw -padx 5 -pady "1 5"
            
            set action [$w createwindow]
            set auto [$w give_uservar_value auto]
            set disable [$w give_uservar_value disable]
            destroy $w
            ...
            if { $action == 1 } {
                if { $disable } {
                    ...
                    if { $auto } {
                        set CheckExternalFileModification auto
                    }
                }
            }
        }
...
proc RamDebugger::DropBindingDone { files } {
    ...
    foreach file $files {
        ...
        if { [file extension $file] in ".png .jpg .jpeg" } {
            set ret [paste_image_selection_or_file -fileFrom $file]
            if { $ret } { continue }
        }
        OpenFileF $file
    }
...
proc RamDebugger::AddFileTypeMenu { filetype } {
    ...
    switch $filetype {
        ...
        latex - wiki {
            set err [catch { package require customLib_utils }]
            if { !$err } {
                set menu [list \
                        [list command "&Upload file" {} "Upload file to server" \
                            "" -command [list RamDebugger::process_wiki upload_file $currentfile]] \
                        [list command "&Upload window..." {} "Detailed upload file to server" \
                            "" -command [list RamDebugger::process_wiki upload_window $currentfile]] \
                        [list separator] \
                        [list command "&Go to web page" {} "Go to server webpage" \
                            "" -command [list RamDebugger::process_wiki goto_link $currentfile]] \
                        [list separator] \
                        [list command "&Embed image" {} "embeds image into document" \
                            "" -command [list RamDebugger::embed_image]] \
                        [list command "&Insert date" {} "inserts date into document" \
                            "" -command [list RamDebugger::insert_date]] \
                    ]
        ...
        markdown {
            set menu [list \
                    ...
                    [list command "&Upload window..." {} "Detailed upload file to server" \
                        "" -command [list RamDebugger::process_wiki upload_window $currentfile]] \
                    [list separator] \
                    [list command "&View markdown" {} "compile and visualize markdown" \
                        "ShiftCtrl p" -command [list RamDebugger::process_markdown -to html5 $currentfile]] \
                    [list command "&View markdown section" {} "compile and visualize markdown section" \
                        "ShiftCtrl l" -command [list RamDebugger::process_markdown -to html5 -only_around_line $currentfile]] \
                    [list command "&Markdown to GiD" {} "compile and visualize markdown svgml to gid" \
                        "" -command [list RamDebugger::process_markdown -to gid -only_around_line $currentfile]] \
                    [list command "Markdown to PDF" {} "export to PDF" \
                        "" -command [list RamDebugger::process_markdown -to pdf $currentfile]] \
                    [list command "&Markdown to mediawiki" {} "convert markdown to mediawiki" \
                        "" -command [list RamDebugger::process_markdown -to mediawiki $currentfile]] \
                    [list command "&Markdown to latex" {} "convert markdown to latex" \
                        "" -command [list RamDebugger::process_markdown -to latex $currentfile]] \
                    [list cascad &[_ "Markdown to slides"] {} toslides 0 [list \
                            [list command "&reveal.js" {} "convert markdown to slides reveal.js" \
                                "" -command [list RamDebugger::process_markdown -to revealjs $currentfile]] \
                            [list command "&reveal.js (self-contained)" {} "convert markdown to slides reveal.js self-contained" \
                                "" -command [list RamDebugger::process_markdown -to revealjs_self_contained $currentfile]] \
                            [list command "&reveal.js print" {} "convert markdown to slides reveal.js for printing" \
                                "" -command [list RamDebugger::process_markdown -to revealjs_print $currentfile]] \
                            [list command "&impress.js" {} "convert markdown to slides impress.js" \
                                "" -command [list RamDebugger::process_markdown -to impressjs $currentfile]] \
                            [list command "&powerpoint" {} "convert markdown to slides powerpoint" \
                                "" -command [list RamDebugger::process_markdown -to pptx $currentfile]] \
                            ]] \
                    [list command "&Markdown to mediawiki zip" {} "convert markdown to mediawiki zip" \
                        "" -command [list RamDebugger::process_wiki create_zip_file $currentfile]] \
                    [list separator] \
                    [list command "&Embed image" {} "embeds image into document" \
                        "" -command [list RamDebugger::embed_image]] \
                    [list command "&Insert date" {} "inserts date into document" \
                        "" -command [list RamDebugger::insert_date]] \
                    [list separator] \
                    [list checkbutton "Formulas as images" {} "view formulas as images" \
                        "" -command [list RamDebugger::markdown_latex_formulae -toggle] \
                        -variable RamDebugger::markdown_draw_formulae_images] \
                    [list checkbutton "Prefer pandoc" {} "Prefer pandoc" \
                        "" -command [list RamDebugger::prefer_pandoc] \
                        -variable RamDebugger::prefer_pandoc] \
                    ]
            set descmenu_new [linsert $descmenu 30 "&Markdown" all filetypemenu 0 $menu]
            set changes 1
        }
...
proc RamDebugger::add_counter { type tags counters_name } {
    upvar 1 $counters_name counters
    
    if { [regexp {\#([\w:]+)} $tags {} id] } {
        set n [expr {[dict_getd $counters ${type}_counter 0]+1}]
        dict set counters ids $id $n
        dict set counters ${type}_counter $n
        return $n
    }
    return ""
}

proc RamDebugger::process_wiki { args } {
    variable text
    ...
    if { [file extension $file] eq ".md" } {
        set file [process_markdown -from markdown -to mediawiki -open 0 $file]
    }
    
    switch $what {
        upload_file {
            mediawiki::upload_window_file -parent $text -file $file \
                -start_callback RamDebugger::Redefine_debug_log
        }
        upload_window {
            mediawiki::upload_window -parent $text -file $file \
            -start_callback RamDebugger::Redefine_debug_log
        }
        goto_link {
            mediawiki::upload_window_goto_linkF -parent $text -file $file \
                -start_callback RamDebugger::Redefine_debug_log
        }
        create_zip_file {
            set err [catch { mediawiki::create_zip_file $file } ret]
            ViewOnlyTextOrAll -force_all
            TextOutRaiseForce
            TextOutClear
            if { $err } {
                RamDebugger::TextOutInsertRed $ret
            } else {
                RamDebugger::TextOutInsert "created file '$ret'\n"
            }
        }
    }
}
...
proc RamDebugger::open_svgml_drawing { svgfile } {
    
    set cmd1 [list open_document -quit $svgfile]
    set cmd2 [list focus_window]
    set err [catch { socket localhost 3000 } socket]
    if { $err } {
        exec ~/myTclTk/drawp/drawp.exe &
        set socket [socket localhost 3000]
    }
    puts $socket "$cmd1\n\r\n"
    flush $socket
    puts $socket "$cmd2\n\r\n"
...
proc RamDebugger::read_file_utf8 { file } {
...
proc RamDebugger::save_file_utf8 { file data } {
...
proc RamDebugger::process0_markdown { args } {
    
    set optional {
        { -only_around_line_num number "" }
        { -numbering boolean 1 }
        { -impressjs boolean 0 }
        { -gid boolean 0 }
        { -transform_for_pandoc boolean 1 }
    }
    set compulsory "file file_out"
    parse_args $optional $compulsory $args
    ...
    if { [regexp {^-----BEGIN MYSECRET-----} $data] } {
        variable files_encryption
        if { [dict exists [set! files_encryption] $file] } {
            set password [dict get $files_encryption $file]
        } else {
            set password [GetFile_encrypt $RamDebugger::text]
        }
        set data [encoding convertfrom utf-8 [exec MySecret.exe -d -@ \
                    -p $password -n << $data]]
        dict set files_encryption $file $password
    }
 
    set counters ""
    set lines [split $data \n]
    
    set image_names [dict create \
            en Image \
            es Imagen \
            cat Imatge \
            ]
    ...
    set is_inside_fenced 0
    for { set i 0 } { $i<[llength $lines] } { incr i } {
        set line [lindex $lines $i]
        if { [regexp {^\s*lang:\s+(\w+)\s*$} $line {} l] } {
            set lang $l
        }
        if { [regexp {^\s*engine:\s+(\S+)\s*$} $line {} l] } {
            dict set return_dict engine $l
        }
        if { [regexp {^\s*my-background-image:\s+(\S+)\s*$} $line {} l] } {
            dict set return_dict my-background-image $l
        }
        
        if { $only_around_line_num ne "" && [regexp {^\s*(# |~~~)} $line what] } {
            if { $what eq "# " } {
                if { [info exists fenced] } { continue }
            } elseif { ![info exists fenced] } {
                set fenced 1
            } else {
                unset fenced   
            }
            if { $i < $only_around_line_num } {
                set line_start $i
                if { [info exists fenced] } {
                    set is_inside_fenced 1
                }
            } elseif { ![info exists line_end] } {
                if { [regexp {^\s*~~~} $line] } {
                    if { $is_inside_fenced } {
                        set line_end $i
                    } else {
    ...
    set is_svgml_alone 0
    if { [info exists line_start] || [info exists line_end] } {
        if { ![info exists line_start] } { set line_start 0 }
        if { ![info exists line_end] } { set line_end [llength $lines] }
        set lines [lrange $lines $line_start $line_end]
        
        if { [regexp {^\s*~{3,}.*file=['"](.*)['"]} [lindex $lines 0] {} svgfile] } {
            set lines [linsert $lines 0 "!\[\]($svgfile)\n\n"]
            set is_svgml_alone 1
        }
        if { [regexp {^---\n.*?\n---\n\n} $data header] } {
            set lines [linsert $lines 0 $header]
        } else {
            set lines [linsert $lines 0 "---\ntitle: testing\ncss: css/markdown.css\n---\n\n"]
        }
    }
    if { $only_around_line_num ne "" && [dict_getd $return_dict engine ""] eq "reveal.js" } {
        set transform_for_pandoc 1
    }
    
    if { $transform_for_pandoc } {
        for { set i 0 } { $i<[llength $lines] } { incr i } {
            set line [lindex $lines $i]
            set l0 $i
            set l_end $i
            if { $i>0 && [regexp -indices {^\s*([\{].+?[\}])\s*$} $line all b t] } {
                # ids are in an isolated line after a normal line
                set line "[lindex $lines $i-1]\n$line"
                incr l0 -1
            }
            if { [regexp -indices {^(.*)([\{].+?[\}])\s*$} $line all b t] } {
                set before [string range $line {*}$b]
                set tags [string range $line [lindex $t 0]+1 [lindex $t 1]-1]
                if { [regexp {^\s*(\$\$?.+?\$?\$)\s*$} $before {} f] } {
                    # $ formula $ {#formula:15}
                    set n [add_counter formula $tags counters]
                    append tags " .MDformula"
                    set table "::: {$tags}\n"
                    append table "|\n"
                    append table "|:---:|---:|\n"
                    append table "|$f|($n)|\n"
                    append table ":::"
                    set lines [lreplace $lines $l0 $l_end $table]
                } elseif { [regexp {(?i)(?:^\s*Table:\s*(\S.*\S)\s*|[^\\][|].*)$} $before {} f] } {
                    # Table: .... {#table:15}
                    add_counter table $tags counters
                    for { set j [expr {$l0-1}] } { $j >= 0 } { incr j -1 } {
                        set lineL [lindex $lines $j]
                        if { [regexp {^\s*$} $lineL] && $j==[expr {$l0-1}] } { continue }
                        if { [regexp {(^\||[^\\]\|)} $lineL]==0 } { break }
                    }
                    incr j
                    append tags " .MDtable"
                    set table "::: {$tags}\n"
                    for { set k $j } { $k <=$i-1 } { incr k } {
                        append table "[lindex $lines $k]\n"
                    }
                    append table $before "\n"
                    append table ":::"
                    set lines [lreplace $lines $j $l_end $table]
                    set i $j
                } elseif { [regexp {^\s*(\!\[[^][]*\][\(][^][()]*[\)])\s*$} $before {} f] } {
                    # ![image desc](image link) {image:15}
                    add_counter image $tags counters
                    
                    regexp {^\s*\!\[([^\]]*)\](.*)} $line {} txtG rest
                    regsub {\)\s+\{} $rest ")\{" rest
                    if { [regexp {\.numbered} $tags {} id] } {
                        regexp {\#([\w:]+)} $tags {} id
                        if { [dict exists $image_names $lang] } {
                            set I [dict get $image_names $lang]
                        } else {
                            set I [dict get $image_names en]
                        }
                        set txtG "**$I @$id**. $txtG"
                    }
                    set line "!\[$txtG\]$rest"
                    set lines [lreplace $lines $l0 $l_end $line]
                    set i $l0
                    
                    #                 set lines [lreplace $lines $l0 $l_end $before]
                    #                 set i $l0
                } elseif { [regexp {^\s*(\[[^][]*\]:\s.*)\s*$} $before {} f] } {
                    # [image tag]: image link {image:15}
                    # there is another case below for when it is broken into several lines
                    add_counter image $tags counters
                } elseif { [regexp {^\s*~{3,}} $before] } {
                    # ~~~~ { svgml="1" file="images/boxes_desc.svg" }
                    if { ![regexp {svgml} $tags] } { continue }
                    set found 0
                    for { set j [expr {$i+1}] } { $j<[llength $lines] } { incr j } {
                        if { [regexp {^\s*~{3,}} [lindex $lines $j]] } {
                            set found 1
                            break
                        }
                    }
                    if { $found } {
                        if { [regexp {file=['"](.*)['"]} $tags {} svgfile] } {
                            set svgfile [file join [file dirname $file] $svgfile]
                        } else {
                            set fin [file tempfile svgfile .svg]
                            close $fin
                        }
                        set txt [join [lrange $lines $i+1 $j-1] "\n"]
                        
                        # can be gid,new_app,old_app
                        set what "new_app" 
                        
                        if { $what eq "gid" } {
                            RamDebugger::Instrumenter::TryLoadLibrary
                            set gid_batch [svgml_evaluate gid $txt]
                            set file [file root $svgfile].bch
                            save_file_utf8 $file $gid_batch
                            set drawpost "mytcltk/gid_groups_conds_dir/src/full/full.tcl"
                            exec tclsh $::env(HOME)/$drawpost -pre -p compassfem -b $file &
                        } elseif { $what eq "new_app" } {
                            RamDebugger::Instrumenter::TryLoadLibrary
                            set svg [svgml_evaluate svg $txt]
                            save_file_utf8 $svgfile $svg
                        } elseif { $what eq "old_app" } {
                            svgml::create_file -file $svgfile $txt
                        }
                        lappend svgFiles $svgfile
                        set lines [lreplace $lines $i $j]
                        incr i -1
                    }
                } else {
                    for { set j [expr {$l0-1}] } { $j >= 0 } { incr j -1 } {
                    ...
                    if { [regexp {^\s*\[[^\]]*\]:\s+} [lindex $lines $j]] } {
                        # [image tag]: image link {image:15}
                        # this is the case when broken into several lines
                        add_counter image $tags counters
                        set txt ""
                        for { set k $j } { $k < $l_end } { incr k } {
                            append txt [lindex $lines $k]
                        }
                        set lines [lreplace $lines $j $l_end $txt]
                        set i $j
                        continue
                    } elseif { [regexp {^\s*\[[^\]]*\]\s+\w} [lindex $lines $j]] } {
                        # [ref] ref text {image:15}
                        set n [add_counter reference $tags counters]
                        append tags " .MDreference"
                    } else {
                        continue
                    }
                    set txt "::: {$tags}\n"
                    for { set k $j } { $k <=$i-1 } { incr k } {
                        if { $k == $j && $n ne "" } {
                            regexp {^\s*\[[^\]]*\]\s+(.*)} [lindex $lines $j] {} txtL
                            append txt "\[$n\] $txtL"
                        } else {
                            append txt "[lindex $lines $k]\n"
                        }
        ...
        if { $numbering } {   
            ...
            for { set i 0 } { $i < [llength $lines] } { incr i } {
                set line [lindex $lines $i]
                if { [regexp {^\s*(#{1,6})\s+(.*)$} $line {} m rest] } {
                    if { [regexp {(?i)^references$} $rest] } { continue }
                    set level [string length $m]
                    if { $l0 eq "" } {
                        set l0 $level
                    }
                    dict incr countersH $level
                    for { set l [expr {$level+1}] } { 1 } { incr l } {
                        if { ![dict exists $countersH $l] } {
                            break
                        }
                        dict unset countersH $l
                    }
                    set line "$m "
                    for { set l $l0 } { $l <= $level } { incr l } {
                        if { $l > $l0 } { append line "." }
                        if { ![dict exists $countersH $l] } {
                            dict set countersH $l 1
                        }
                        append line [dict get $countersH $l]
                    }
                    append line " " $rest
                    set lines [lreplace $lines $i $i $line]
                }
            }
        }
    }
    
    if { [dict exists $return_dict my-background-image] } {
        set txt "{ data-background-image=\"[dict get $return_dict my-background-image]\" "
        append txt "data-background-size=\"100% 100%\" }"

        for { set i 0 } { $i < [llength $lines] } { incr i } {
            set line [lindex $lines $i]
            if { [regexp {^\s*((#{1,6})\s+(.*)|-{4,})$\s*} $line] } {
               append line " $txt"
                lset lines $i $line
            }
        }
    ...
    if { $impressjs } {
        ...
        set r [expr {1000*$num/2.0/3.14}]
        set angle [expr {2*3.14/$num}]
        set idx 0
        for { set i 0 } { $i < [llength $lines] } { incr i } {
            set line [lindex $lines $i]
            set x [expr {$r*cos($idx*$angle)}]
            set y [expr {$r*sin($idx*$angle)}]
            set angleD [expr {($idx*$angle)*180/3.14}]
            if { [regexp {^\s*(#{1,6})\s+(.*)$} $line {} m rest] } {
                set line "# $rest {.step .slide data-x=$x data-y=$y data-rotate=$angleD }"
                set lines [lreplace $lines $i $i $line]
                incr idx
            } elseif { [regexp {^\s*-{4,}\s*$} $line] } {
                set line "# {.step .slide data-x=$x data-y=$y data-rotate=$angleD }"
                set lines [lreplace $lines $i $i $line]
                incr idx
            }
        }
        set scale [expr {$r/1000.0+5}]
        set line "# {#overview .step .slide data-x=0 data-y=0 data-scale=$scale }"
        set lines [linsert $lines $line0 $line]
    }
    
    set data [join $lines \n]
    
    set idx 0
    while { [regexp -start $idx -line -indices {@([\w:]+)} $data all b] } {
        set id [string range $data {*}$b]
        if { [dict exists $counters ids $id] } {
            set num [dict get $counters ids $id]
            set data [string replace $data {*}$all $num]
            set idx [expr {[lindex $all 0]+[string length $num]}]
        } else {
            set idx [expr {[lindex $all 1]+1}]
        }
    }
    ...
    dict set return_dict svgFiles $svgFiles
    return $return_dict
}

proc RamDebugger::process_markdown { args } {
    variable text
    variable markdown_data
    variable prefer_pandoc
    
    set optional {
        { -from markdown "markdown" }
        { -to mediawiki|html|html5|pdf|latex|s5|revealjs|revealjs_self_contained|revealjs_print|impressjs|pptx|gid "html5" }
        { -open boolean 1 }
        { -only_around_line "" 0 }
    }
    set compulsory "file"
    ...
    if { ![info exists markdown_data] } {
        set markdown_data ""
    }
    
    if { $only_around_line } {
        regexp {^\d+} [$text index insert] only_around_line_num
    } else {
        set only_around_line_num ""
    }
    
    set transform_for_pandoc 1
    if { $to in "html5 pdf" && [set! RamDebugger::prefer_pandoc]==0 } {
        set transform_for_pandoc 0
    }
    
    if { $to eq "html" } {
        if { [auto_execok multimarkdown] eq "" } {
            set url "https://fletcherpenney.net/multimarkdown"
            snit_messageBox -message [= "It is necessary to install program 'multimarkdown' (%s)" $url]
            return
        }
    } elseif { $transform_for_pandoc } {
        if { [auto_execok pandoc] eq "" } {
            set url "https://pandoc.org"
            snit_messageBox -message [= "It is necessary to install program 'pandoc' (%s)" $url]
            return
        }
    }
    ...
    set pwd [pwd]
    cd [file dirname $file]
    close [file tempfile tmpfileName]
    set fileTo $file
    
    set numbering 1
    if { $to in "s5 revealjs revealjs_self_contained revealjs_print pptx" } {
        set numbering 0
    }
    set impressjs 0
    if { $to eq "impressjs" } {
    ...
    set return_dict [process0_markdown -numbering $numbering \
            -only_around_line_num $only_around_line_num \
            -impressjs $impressjs -transform_for_pandoc \
            $transform_for_pandoc -gid $gid $file $tmpfileName]
    set svgFiles [dict get $return_dict svgFiles]

    if { 0 && $only_around_line && [llength $svgFiles] == 1 } {
        open_svgml_drawing [lindex $svgFiles 0]
        return
    }
    if { $to eq "gid" } {
        return
    ...
    if { $only_around_line && [dict_getd $return_dict engine ""] eq "reveal.js" } {
        set to revealjs
    }
    
    switch $to {
        html {
            set fileTo [file root $fileTo].html
            exec multimarkdown $tmpfileName -o $fileTo
            if { $open } {
                cu::file::execute start $fileTo
                #dict set markdown_data files_to_delete $fileTo
            }
        }
        html5 {
            set fileTo [file root $fileTo].html
            if { [set! RamDebugger::prefer_pandoc] } {
                exec pandoc -s --mathjax --from $from --to $to \
                    $tmpfileName -o $fileTo
            } else {
                RamDebugger::Instrumenter::TryLoadLibrary
                set data [read_file_utf8 $tmpfileName]
                set html [markdown_evaluate html $data]
                save_file_utf8 $fileTo $html
            }
            if { $open } {
                cu::file::execute start $fileTo
                #dict set markdown_data files_to_delete $fileTo
        ...
        pdf {
            set fileTo [file root $fileTo].html
            if { [set! RamDebugger::prefer_pandoc] } {
                exec pandoc -s --mathjax --from $from --to html5 $tmpfileName -o $fileTo
            } else {
                RamDebugger::Instrumenter::TryLoadLibrary
                set data [read_file_utf8 $tmpfileName]
                set html [markdown_evaluate html $data]
            ...
            set parameters "<fields><field n='column_number' v='1'/></fields>"
            exec ~/myTclTk/drawp/drawp.exe -no_window -cmd \
                "open_document -pdf -quit -parameters \"$parameters\" \"$fileTo\""  
            #dict set markdown_data files_to_delete $fileTo
        }
        s5 {
            variable topdir
            set dir [file join $topdir addons s5 default]
            set fileTo [file root $fileTo].html
            exec pandoc -s -t s5 --mathjax --from $from --to $to $tmpfileName -o $fileTo \
                -V s5-url=$dir
            if { $open } {
                cu::file::execute start $fileTo
                dict set markdown_data files_to_delete $fileTo
        ...
        revealjs - revealjs_self_contained - revealjs_print {
            variable topdir
            set dir [file join $topdir addons reveal.js]
            set fileTo [file root $fileTo].html
            if { $to eq "revealjs_print" } {
                set theme white
            } else {
                set theme scipedia
            }
            ...
            set to0 $to
            if { $to eq "revealjs_self_contained" } {
                set to revealjs
                set selfcontained_CMD [list --self-contained]
            } elseif { $to eq "revealjs_print" } {
                set to revealjs
                set selfcontained_CMD ""
            } else {
                set selfcontained_CMD ""
            }
            exec pandoc -s -t revealjs \
                -V theme=$theme -V progress=true -V slideNumber=true -V height=800 \
                -V mouseWheel=true -V revealjs-url=$dir -V controls=true -V hideAddressBar=true \
                -V hash=false -V history=false \
                --mathjax=[file join $topdir addons mathjax tex-chtml.js] \
                {*}$selfcontained_CMD --from $from --to $to $tmpfileName -o $fileTo

            if { $open } {
                if { $to0 eq "revealjs_print" } {
                    set key {HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\Windows\CurrentVersion\App Paths\chrome.exe}
                    set chrome [registry get $key ""]
                    cu::file::execute exec $chrome file://$fileTo?print-pdf
                } else {
                    cu::file::execute start $fileTo
                }
                #dict set markdown_data files_to_delete $fileTo
        ...
        impressjs {
            variable topdir
            set dir [file join $topdir addons impress.js]
            set fileTo [file root $fileTo].html
            exec pandoc -s -t html5 \
                --template $dir/impress-template.html \
                -V impress-url=$dir --section-divs \
                --from $from $tmpfileName -o $fileTo
        ...
        pptx {
            set fileTo [file root $fileTo].pptx
            exec pandoc -s -t pptx --mathjax --from $from --to $to $tmpfileName -o $fileTo
            if { $open } {
                cu::file::execute start $fileTo
                dict set markdown_data files_to_delete $fileTo
            }
        ...
        mediawiki {
            set fileTo [file root $fileTo].wiki
            exec pandoc -s --from $from --to $to $tmpfileName -o $fileTo
            if { $open } {
                RamDebugger::OpenFileF $fileTo
            }
        }
        latex {
            set fileTo [file root $fileTo].tex
            exec pandoc -s --from $from --to $to $tmpfileName -o $fileTo
            if { $open } {
                RamDebugger::OpenFileF $fileTo
            }
    ...
    cd $pwd
    process_markdown_check_delete_tmpfiles
    return $fileTo
}

proc RamDebugger::process_markdown_check_delete_tmpfiles { args } {
    variable markdown_data
    ...
    after cancel [dict_getd $markdown_data files_to_delete_afterid ""]
    ...
    set files_to_delete [dict_getd $markdown_data files_to_delete ""]
    dict set markdown_data files_to_delete ""
    set num 0
    foreach file $files_to_delete {
        if { ![file exists $file] } { continue }
        if { $force || $now-[file mtime $file] > 60 } {
            file delete $file
        } else {
            dict lappend markdown_data files_to_delete $file
            incr num
        }
    }
    if { $num > 0 } {
        set id [after 10000 [list RamDebugger::process_markdown_check_delete_tmpfiles]]
        dict set markdown_data files_to_delete_afterid $id
    } else {
        dict set markdown_data files_to_delete_afterid ""
    }
}

proc RamDebugger::prefer_pandoc { args } {
    variable prefer_pandoc
    
    #set prefer_pandoc [expr {($prefer_pandoc)?0:1}]
}

proc RamDebugger::markdown_latex_formulae { args } {
    variable text
    variable currentfile
    variable markdown_data
    variable markdown_draw_formulae_images
    variable AppDataDir
    variable currentfileIsModified
    ...
    if { ![info exists markdown_data] } { set markdown_data "" }
    
    if { ![dict exists $markdown_data markdown_draw_formulae_images] } {
        dict set markdown_data markdown_draw_formulae_images 1
    }    
    if { $toggle } {
        if { [dict get $markdown_data markdown_draw_formulae_images] } {
            dict set markdown_data markdown_draw_formulae_images 0
        } else {
            dict set markdown_data markdown_draw_formulae_images 1
        }
    }
    set markdown_draw_formulae_images [dict get $markdown_data markdown_draw_formulae_images]
    
    set after_id [dict_getd $markdown_data after_id ""]
    after cancel $after_id
    
    if { [GiveFileType $currentfile] ne "markdown" } {
        return
    }
    
    if { $markdown_draw_formulae_images == 0 } {
        while 1 {
            lassign [$text dump -image 1.0 end] key value index
            if { $key eq "" } { break }
            if { $key eq "image" } {
                set currentfileIsModified_save $currentfileIsModified
                $text delete $index
                lassign [$text tag nextrange elide $index] idxE1 idxE2
                $text tag remove elide $idxE1 $idxE2
    ...
    set idx1 [$text index @0,0]
    set idx2 [$text index @0,[winfo height $text]]
    set idx_insert [$text index insert]
    
    lassign [$text dump -image $idx_insert] key value index
    if { $key eq "image" } {
        set currentfileIsModified_save $currentfileIsModified
        $text delete $index
        lassign [$text tag nextrange elide $index] idxE1 idxE2
    ...
    while 1 {
        set idx [$text search -count ::n -regexp {\$\$?[^$]+\$?\$} $idx1 $idx2]
        if { $idx eq "" } { break }
        if { [$text compare $idx <= insert] && [$text compare "$idx+$::n c" > insert] } {
            set idx1 [$text index "$idx+[expr {$::n+1}] c"]
        } else {
            break   
        }
    }
    if { $idx ne "" } {
        regexp {\$\$?([^$]+)\$?\$} [$text get $idx "$idx+$::n c"] {} formula
        if { ![dict exists $markdown_data f_to_img $formula] } {
            if { ![dict exists $markdown_data formula_db] } {
                set file [file join $AppDataDir formula_db.sqlite]
                package require sqlite3
                sqlite3 db_sqlite $file
                dict set $markdown_data formula_db db_sqlite
                db_sqlite eval { create table if not exists formulas(name primary key,data) }
            }
            set data [db_sqlite onecolumn { select data from formulas where name=$formula }]
            if { $data eq "" } {
                package require tls
                package require http
                ::http::register https 443 ::tls::socket
                set url "https://latex.codecogs.com/png.latex?[http::formatQuery $formula]"
                set h [http::geturl $url]
                set data [http::data $h]
                http::cleanup $h
                db_sqlite eval { insert into formulas(name,data) values($formula,$data) }
            }
            set img [image create photo -data $data]
            dict set markdown_data f_to_img $formula $img
            dict set markdown_data img_to_f $img $formula
        }
        set img [dict get $markdown_data f_to_img $formula]
        if { [image width $img] > 1 } {
            set currentfileIsModified_save $currentfileIsModified
            $text tag configure elide -elide 1
            $text tag add elide $idx "$idx+$::n c"
            $text image create $idx -image $img
            $text tag add $img $idx
            $text tag bind $img <ButtonRelease-1> "[list $text mark set insert $idx]; break"
            if { $currentfileIsModified && !$currentfileIsModified_save } {
                MarkAsNotModified
            }
        }
...
proc RamDebugger::embed_image {} {
    variable text
    variable currentfile
    
    set rexA {[-\w/\\.]+\.(svg|png)}
    set rexB {<data:image/(\w+);base64,([\w=+/\s]+)>}
    
    set txt [$text get "insert linestart" "insert lineend"]
    if { [regexp -indices $rexA $txt idxs] } {
        set img [string range $txt {*}$idxs]
        set file [file join [file dirname $currentfile] $img]
        if { ![file exists $file] } {
            tk_messageBox -message "Image '$img' not found"
            return
        }
        ...
        set txt [string range $txt 0 [lindex $idxs 0]-1]
        set type [string tolower [string trim [file extension $file] .]]
        set rest [string range $txt [lindex $idxs 1]+1 end]
        append txt "<data:image/$type;base64,$base64>"
        append txt $rest
        $text replace "insert linestart" "insert lineend" $txt
    } elseif { [set idx [$text search -regexp -count ::count $rexB "insert linestart"]] ne "" } {
        set txt [$text get $idx "$idx + $::count chars"]
        set rex3 {<data:image/(\w+);base64,([\w=+/\s]+)>}
        regexp -indices {<data:image/(\w+);base64,([\w=+/\s]+)>} $txt idxs type_i b_i
        set type [string range $txt {*}$type_i]
        set b [string range $txt {*}$b_i]
        set data [binary decode base64 $b]
        set file [tk_getSaveFile -initialdir [file dirname $currentfile] \
                -initialfile image.$type -title [_ "Save image"]]
        if { $file eq "" } { return }
        set fout [open $file wb]
        puts -nonewline $fout $data
        close $fout
        set file [file normalize $file]
        set file0 [file normalize [file dirname $currentfile]]/
        if { [string equal -len [string length $file0] $file0 $file] } {
            set file [string range $file [string length $file0] end]
        }
        set txt "$file"
        $text replace $idx "$idx + $::count chars" $txt
    } else {
        tk_messageBox -message "cursor should be over an image name"
        return
    }
    SetIsModified
...
proc RamDebugger::insert_date {} {
    variable text
    
    $text insert insert [clock format [clock seconds] -format "%Y-%m-%d"]
}
...
proc RamDebugger::ExtractExamplesDir {} {
    ...
    RamDebugger::SetMessage [_ "Copied examples directory into directory '%s'" $dir]
}
    ...
    if { !$iswince } {
        proc ::bgerror { errstring } {
            if { [info commands RamDebugger::TextOutRaise] != "" } {
                ::tk::dialog::error::bgerror $errstring
                RamDebugger::TextOutRaise
                RamDebugger::TextOutInsertRed "-------------ERROR FROM RAMDEBUGGER-----------\n"
                RamDebugger::TextOutInsertRed $::errorInfo
                RamDebugger::TextOutInsertRed "----------------------------------------------\n"
                #WarnWin "$errstring" ;avoid this message and call standard ::tk::dialog::error::bgerror, not not lost stack trace
...
#    grid $fulltext.can -bg grey90 -grid "0 wns"
    ...
    proc TextOutRaiseForce {} [list TextOutRaiseDoForce $pane2in2.nb]
        ...
        bind all <MouseWheel> {
            ...
            while { $w != [winfo toplevel $w] } {
                    ...
                    if { $ycomm != "" } {
                        set D "%D"
                        if { abs($D)<36 } {
                            set D [expr {1.0*$D/abs($D)*36}]
                        }
                        set units [expr int(-1*$D/36)]
                        $w yview scroll $units units
                        break
                    }
                }
...
proc RamDebugger::OpenDefaultFile {} {
    ...
    if { [info exists options(currentfile)] && $options(currentfile) != ""  && \
        ...
        RamDebugger::SetMessage [_ "Opening file '%s'" $options(currentfile)]...
