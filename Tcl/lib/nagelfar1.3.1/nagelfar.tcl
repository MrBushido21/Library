...
# Try to show a file using Sublime
proc trySubl {filename lineNo} {
    if {$lineNo > 0} {
        if {[catch {exec [auto_execok subl] $filename:$lineNo &}]} {
            return 0
        }
    } elseif {[catch {exec [auto_execok subl] &}]} {
        return 0
    }
    return 1
}
...
proc editFile {filename lineNo} {
    ...
    if {$::Prefs(editor) eq "subl"  && [trySubl  $filename $lineNo]} return
...
proc addOptionsMenu {m} {
    ...
    $m.mo.med add radiobutton -label "Sublime" \
            -variable ::Prefs(editor) -value subl
