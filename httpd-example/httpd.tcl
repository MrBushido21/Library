...
namespace eval ::httpd {
    ...
    proc httpd {command args} {
        ...
        switch -exact $command {
            ...
            "stop" {
                if {$argc == 1 && [lindex $args 0] eq "-ssl"} {
                    if {[info exists Httpd(listenssl)]} {
                        catch {close $Httpd(listenssl)}
                        unset Httpd(listenssl)
                        HttpdClearError
                    }
                    return ""
                } elseif {$argc == 0} {
                    if {[info exists Httpd(listen)]} {
            ...
            "loadrequest" {
                if {$argc == 3} {
                    ...
                    uplevel 1 array set [list [lindex $args 2] {}]
                    HttpdClearError
                    set query ""
                    if {[info exists data(query)]} {
                        set query $data(query)
                    }
                    if {[info exists data(proto)] && $data(proto) eq "POST" \
                    ...
                    foreach {x} [split [string trim $query] &] {
                        ...
                        if {$posn != -1 && $posn == [string last "=" $x]} {
                            ...
                            uplevel 1 set [list [lindex $args 2]($varname)] [list [HttpdDecode $val]]
                        }
                    }
                } elseif {$argc == 2} {
                    uplevel 1 upvar #0 Httpd[lindex $args 0] [lindex $args 1]
            ...
            "headers" {
                if {$argc == 1} {
                    ...
                    upvar #0 Httpd$sock data
                    set headers {}
                    foreach mime [array names data mime,*] {
                        lappend headers [string range $mime 5 end] $data($mime)
                    }
                    return $headers
                } else {
                    error "wrong # args, should be: httpd headers sock"
                }
            }
            "decode" {
                if {$argc == 1} {
            ...
            default {
                    error "invalid httpd command '$command'" 
            }
        }
    }
    ...
    proc Httpd_Server {port responsehandler {address {}} {protocol {}}} {
        ...
        if {$protocol ne "ssl" && $protocol ne ""} {
            error "bad protocol"
        }
        if {$protocol eq "ssl"} {
            ...
            if {$address ne ""} {
                set Httpd(listenssl) [tls::socket -server [list [namespace current]::HttpdAccept https] -myaddr $address $port]
            } else {
                set Httpd(listenssl) [tls::socket -server [list [namespace current]::HttpdAccept https] $port]
            }
            ...
            array set Httpd [list accepts 0 requests 0 errors 0]
            if {$address ne ""} {
                set Httpd(listen) [socket -server [list [namespace current]::HttpdAccept http] -myaddr $address $port]
            } else {
                set Httpd(listen) [socket -server [list [namespace current]::HttpdAccept http] $port]
            }
    ...
    proc HttpdAccept {protocol sock ipaddr {port {}} {protocol {}}} {
        ...
        if {$protocol eq "https"} {
            ...
            fileevent $sock readable [list [namespace current]::HttpdHandshake $sock]
        } else {
    ...
    proc HttpdReset {sock protocol left {ipaddr ""}} {
        ...
        if {$ipaddr ne ""} {set data(ipaddr) $ipaddr}
        ...
        fileevent $sock readable [list [namespace current]::HttpdRead $sock]
    }
                    ...
                    if {$data(proto) eq "POST" \
                        ...
                        if {[info exists data(mime,content-type)] \
                            ...
                            if {$data(version) && [info exists data(mime,expect)]} {
                                if {$data(mime,expect) eq "100-continue"} {
    ...
    proc HttpdSockDone {sock close} {
        ...
        if {[info exists data(cancel)] && $data(cancel) ne ""} {
        ...
        Httpd_Log $sock SockDone return
        return ""
    }
    ...
    proc HttpdRespond {sock} {
        ...
        if {[info exists Httpd(responsehandler)]} {
            ...
            #   }]; puts stderr "-->$timer $url"
                ...
                if {$errmsg eq "websocket"} {return}
                unset -nocomplain data(inprogress)
                if {$errmsg eq "pending"} {
    ...
    proc HttpdCopyDone {in sock close bytes {error {}}} {
        ...
        Httpd_Log $sock HttpdCopyDone $bytes bytes
        HttpdSockDone $sock $close
        set ::Httpd${sock}(final) ""
        update idle
    }
    ...
    proc HttpdError {sock code args} {
        upvar #0 Httpd$sock data
        variable Httpd 
        variable HttpdErrors 
        variable HttpdErrorFormat

        append data(url) ""
        set url $data(url)
        set version "1"; if {[info exists data(version)] && $data(version) ne ""} {set version $data(version)}
        incr Httpd(errors)

        set message [format $HttpdErrorFormat $code $HttpdErrors($code) [HttpdHtmlEncode $url]]
        set length [string length $message]
        incr length 2
        append head "HTTP/1.$version $code $HttpdErrors($code)"  \n
        append head "Date: [HttpdDate [clock seconds] 1]"  \n
        append head "Connection: close"  \n
        append head "Content-Length: $length"  \n
        ...
        HttpdSockDone $sock 1                                                   
        Httpd_Log $sock Error $code $HttpdErrors($code) $args $reason $url
    }
    ...
    proc HttpdHtmlEncode {string} {
        ...
        regsub -all {([$<>&"'])} $string {$entity(\1)} string ;#"
        return [subst -nocommand -nobackslash $string]
    }
    ...
    proc HttpdReturn {sock body {mimetype "text/html"} {staticseconds ""}} {
        ...
        puts $sock "Date: [HttpdDate [clock seconds] 1]"
        ...
        upvar #0 Httpd$sock data
        if {[info exists data(outputheaders)] && $data(outputheaders) ne ""} {
    ...
    proc HttpdReturnRedirect {sock url} {
        ...
        puts $sock ""
        puts $sock [format {Redirect to <a href="$url">%1$s</a>} $url]
    ...
    proc HttpdReturnFile {sock file filename mimetype timestamp {view ""} {flag ""}} {
        ...
        Httpd_Log $sock HttpdReturnFile start $file $filename $mimetype $timestamp $view $flag
        ...
        puts $sock "Last-Modified: [HttpdDate $timestamp 1]"

        if {$flag eq "-static"} {
            puts $sock "ETag: \"$timestamp\""
            puts $sock "Expires: [HttpdDate [expr $timestamp+86400] 1]"
            puts $sock "Cache-Control: max-age=86400, must-revalidate"
        }
        puts $sock "Content-Type: $mimetype"
        ...
        if {$data(proto) ne "HEAD"} {
            ...
            fcopy $in $sock -size $size \
                    -command [list [namespace current]::HttpdCopyDone $in $sock $close]
            vwait ::Httpd${sock}(final)
        } else {
            HttpdSockDone $sock $close
        }
        Httpd_Log $sock HttpdReturnFile done
    }

    proc HttpdReturnFilex {sock file filename mimetype timestamp} {
        upvar #0 Httpd$sock data
        ...
        puts $sock "Last-Modified: [HttpdDate $timestamp 1]"
        puts $sock "Content-Type: $mimetype"
        puts $sock "Content-Length: [file size $file]"
        set close 1
        if {$close} {
            puts $sock "Connection: close" ;#jcw
        }
        puts $sock ""
        flush $sock

        if {$data(proto) ne "HEAD"} {
            puts "copy $file [file size $file] "
            fconfigure $sock -translation binary
            set in [open $file]
            fconfigure $in -translation binary
            set sz [file size $file]
            fcopy $in $sock -size $sz -command [list [namespace current]::HttpdCopyDone $in $sock $close]
            vwait ::Httpd${sock}(final)
        } else {
            HttpdSockDone $sock $close
        }
    ...
    proc HttpdUploadInit {sock} {
        ...
        fileevent $sock readable [list [namespace current]::HttpdUploadFindBoundary $sock]
    }
    ...
    proc HttpdUploadFindBoundary {sock} {
        ...
        if {[gets $sock line] > 0} {
            ...
            if {[string first "--$data(upload-boundary)" $line] == 0} {
                fileevent $sock readable [list [namespace current]::HttpdUploadReadHeader $sock]
            }
        }
    }
    ...
    proc HttpdUploadReadHeader {sock} {
        ...
        while {[gets $sock line]>=0} {
            ...
            if {[string length [string trim $line]] == 0} {
                ...
                if {[info exists data(upload-filename)] && $data(upload-filename) ne ""} {
                    set data(formvar-${varname}-filename) \
                            [HttpdDecode $data(upload-filename)]
                }
                if {![info exists data(upload-filename)] || $data(upload-filename) eq ""} {
                    fileevent $sock readable [list [namespace current]::HttpdUploadReadPart $sock]
                } else {
                    ...
                    set fn "upload-$data(formvar-${varname}-filename)"
                    ...
                    fileevent $sock readable [list [namespace current]::HttpdUploadReadFile $sock]
                }
                return
            }
                ...
                if {[string equal $hdrname "content-disposition"]} {
                    foreach {x} [split [string trim $value] ";"] {
                        if {[regexp -- {(.*)=\"*([^\"]*)\"*\;*} [string trim $x] dummy var val]} {
                            if {[string tolower $var] eq "name"} {
                                set data(upload-formname) $val
                            } elseif {[string tolower $var] eq "filename"} {
    ...
    proc HttpdClearError {} {
        foreach {::errorInfo ::errorCode} {"" 0} break
    }
}
